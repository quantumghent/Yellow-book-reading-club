...
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{4\pi K}\int d^2x \ \left(i\psi_{+} (\partial t - \partial x) \psi_{+} + i \psi_{-}(\partial t + \partial x)\ \psi_{-} \right). \\		
		&= \frac{1}{4\pi K}\int d^2x \ \left(i\psi_{+} \partial_{-} \psi_{+} + i \psi_{-} \partial_{+} \psi_{-} \right). 
	\end{aligned}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{4\pi K} \int d^2x \ \left ( i \psi^\dagger \partial_t \psi - i\psi^\dagger \sigma^z \partial_x \psi \ \right) \\
    &= \frac{1}{4\pi K} \int d^2x \ i \psi^\dagger \left( \partial_t - \sigma^z\partial_x\right)\psi \\
    &= \frac{1}{4\pi K} \int d^2x \ i \psi^\dagger \left( \sigma^x \sigma^2 \partial_t + \sigma^x i\sigma^y \partial_x\right)\psi \\
	&= \frac{1}{4\pi K} \int d^2x \ \psi^\dagger i \gamma^0 \gamma^\mu \partial_\mu \psi\\
	&= \frac{1}{4\pi K} \int d^2x \ \bar{\psi} \, i \slashed{\partial} \psi
	\end{aligned}
\end{equation}
where we used
\begin{equation}
...
\begin{equation}
	S = \frac{1}{4\pi K} \int d^2x \ \bar{\psi} \, \left( i \slashed{\partial} - m \right) \psi.
\end{equation}
...
\begin{equation}
	\begin{split}
		\phi(z) &= \sqrt{K}\left(\phi_0 -i\,j_0\,\ln \,z + i\,\sum_{n\neq0} \frac{1}{n}\,j_n\,z^{-n}\right) \\
		\bar{\phi}(\bar{z}) &= \sqrt{K} \left(\bar{\phi}_0 -i\,\bar{j}_0\,\ln \, \bar{z} + i\,\sum_{n\neq0} \frac{1}{n}\,\bar{j}_n\,{\bar{z}}^{-n}\right).
	\end{split}
\end{equation} 
...
\begin{equation}
	\begin{split}
		\varphi &= \sqrt{K}\left(\varphi_0 -i \tau \left(j_0 + \bar{j_0}\right) + \sigma \left(j_0 - \bar{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}+ \bar{j}_n\,{\bar{z}}^{-n}\right)\right) \\
		\theta &= \sqrt{K}\left(\varphi_0 -i \tau \left(j_0 - \bar{j_0}\right) + \sigma \left(j_0 + \bar{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}- \bar{j}_n\,{\bar{z}}^{-n}\right)\right).
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\partial_x \varphi &= \partial \varphi\ i z + \bar{\varphi} (-i) z = \sqrt{K} \left( j_n/z^n -\bar{j}_n / \bar{z}^n \right) \\
		4\pi K\pi &= \partial \varphi i z + \bar{\partial} \varphi i \bar{z} = \sqrt{K} \left( j_n/z^n + \bar{j}_n / \bar{z}^n \right) 
	\end{aligned}
\end{equation}
...
Now let's derive the quantization of the zero modes of the current operator. It is clear that $j_0$ and $\bar{j}_0$ are converved quantuties. Let's denote their charge as $\Delta$ or $\bar{\Delta}$. Changing $z$ to $ze^{i2\pi}$ for the scalar field $\varphi$, it should be single valued, which resultes in the relation $\Delta - \bar{\Delta} = n\frac{R}{\sqrt{K}} $
...
\begin{equation}
	\begin{aligned}
		j_0 &= \frac{\sqrt{K}m}{R} + \frac{R n}{2\sqrt{K}} \\
		\bar{j}_0 &= \frac{\sqrt{K}m}{R} - \frac{R n}{2\sqrt{K}}
	\end{aligned}
\end{equation}
One can also introduce its dual field $\theta(z,\bar{z}) := \phi(z) - \bar{\phi}(\bar{z})$. It is compactified on a circle with a different radius 
\begin{equation}
	\theta(x) \sim \theta(x) + 4\pi K /R.	
\end{equation}
This indicates that there are two $O(2)$ symmetry in the free compact boson theory, which correspond to $\varphi(x)$ and $\theta(x)$ respectively.




{\bf Bosonization dictonary for free fermions---}
The simplest way for bosonization of fermions is through the vertext operator $V_\alpha = :e^{i\alpha \phi(z)}:$ with a dimension $(K\alpha^2/2,0)$. Now we can bosonize the fermion fields
\begin{equation}
	\begin{split}
		&\psi(z) \sim e^{i \phi/\sqrt{K}} \\
		&\bar{\psi}(\bar{z}) \sim e^{i\bar{\phi} /\sqrt{K}}.
	\end{split}
\end{equation}
