...
The simplest way for bosonization of fermions is by matching the conformal dimension of the fermion operator to the vertext operator. Take the right mover $\psi_{-}$, a holomorphic field $\left(1/2,0\right)$, as an example
\begin{equation}
	\begin{aligned}
		&e/R + mR/2 = 1\\
		&e/R - mR/2 = 0.
...
\begin{equation}
	E_k \sim -\cos (k),
\end{equation}
one can identify fermions around $\mp k_f$ as the left/right mover $\psi_{+}$/$\psi_{-}$. Note there is another subtle point that we have normalized the correlation of fermion fields, while the canonical anticommutation relation requires the two-point correlation $\frac{1}{2\pi}\frac{1}{z-w}$. Thus one identifies that the slow modes are close to $\pm k_f$ ($k_f > 0$),
\begin{equation}
	\begin{aligned}
		c\left(t,x\right) \sim e^{-ik_f \left( t+x \right)} \psi_{+}/\sqrt{2\pi} + e^{-ik_f\left(t-x\right)} \psi_{-}/\sqrt{2\pi},
	\end{aligned}
\end{equation} 
\textcolor{red}{Should we use $4\pi$? The holomorphic and antiholomorphic both contributes a factor. Is this the fermion doubling problem?}

The lattice fermon density is 
\begin{equation}
	\begin{aligned}
		2\pi\,\rho :=& 2\pi\,c^\dagger c \sim \psi_{+}^\dagger \psi_{+} + \psi_{-}^\dagger \psi_{-} + \left(e^{2i k_f}\psi_{+}^\dagger \psi_{-} + h.c. \right) \\
		=& i \left(\partial \varphi + \overline{\partial} \varphi\right) + 2 \cos (\theta + 2k_f x) \\
		=& \partial_x \theta + 2 \cos (\theta + 2k_f x)
	\end{aligned}
\end{equation}
...
\end{equation}
where $S^\dagger := \sigma^x + i\sigma^y$. Indentifying the up spin state to the empty fermion state, one can use the transformation
\begin{equation}
	\begin{aligned}
