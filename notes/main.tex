...
\begin{eqnarray}
    ...
    \gamma^1 &=& \sigma^y.
\end{eqnarray}
We also usually a slashed notation for $\slashed{\partial} := \gamma^\mu \partial_\mu$.
...
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{4\pi K}\int dxdt \ \left(i\psi_{+} (\partial t - \partial x) \psi_{+} + i \psi_{-}(\partial t + \partial x)\ \psi_{-} \right). \\		
		&= \frac{1}{4\pi K}\int dxdt \ \left(i\psi_{+} \partial_{-} \psi_{+} + i \psi_{-} \partial_{+} \psi_{-} \right). 
	\end{aligned}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{4\pi K} \int dxdt \ \left ( i \psi^\dagger \partial_t \psi - i\psi^\dagger \sigma^z \partial_x \psi \ \right) \\
    &= \frac{1}{4\pi K} \int dxdt \ i \psi^\dagger \left( \partial_t - \sigma^z\partial_x\right)\psi \\
    &= \frac{1}{4\pi K} \int dxdt \ i \psi^\dagger \left( \sigma^x \sigma^2 \partial_t + \sigma^x i\sigma^y \partial_x\right)\psi \\
	&= \frac{1}{4\pi K} \int dxdt \ \psi^\dagger i \gamma^0 \gamma^\mu \partial_\mu \psi\\
	&= \frac{1}{4\pi K} \int dxdt \ \bar{\psi} \, i \slashed{\partial} \psi
	\end{aligned}
\end{equation}
where we used
\begin{equation}
...
\end{equation}
A mass term $m\bar{\psi}\psi = m \left(\psi_{+}^\dagger \psi_{-} +\psi_{-}^\dagger \psi_{+}\right) $ can be added into the action
\begin{equation}
	S = \frac{1}{4\pi K} \int dxdt \ \bar{\psi} \, \left( i \slashed{\partial} - m \right) \psi.
\end{equation}
The massless fermions are Weyl spinors without a mixing between the left and right moving parts. 

{\bf Wick rotation.---}
It is usually more convenient to work in Euclidean space rather than Minkowski space time. Upon Wick rotation, the action changes as ($t \rightarrow i\tau$)
\begin{equation}
...
\begin{equation}
	\begin{aligned}
		-i\,S &= -i\frac{1}{4\pi K}\int dxdt\,\left(\psi_{+}\ i(\partial_t + \partial_x)\ \psi_{+} + \psi_{-}\ i(\partial_t - \partial_x)\ \psi_{-} \right)\\
		&= -i\frac{1}{4\pi K}\int dx d(i\tau) i\left(\psi_{+}\ (\partial_{i\tau} + \partial_x)\ \psi_{+} + \psi_{-}\ i(\partial_{i\tau} - \partial_x)\ \psi_{-} \right) \\
		&= \frac{1}{4\pi K}\int dx d\tau \left(\psi_{+}\ (\partial_\tau + i\partial_x)\ \psi_{+} + \psi_{-}\ (\partial_\tau - i \partial_x)\ \psi_{-} \right) \\
		&= \frac{1}{4\pi K}\int d^2x \,2\left(\psi_{+} \bar{\partial} \psi_{+} + \psi_{-} \partial \psi_{-} \right) \\
		&= \frac{1}{4\pi K}\int d^2z\, \left(\psi_{+} \bar{\partial} \psi_{+} + \psi_{-} \partial \psi_{-} \right) \\
	\end{aligned}
\end{equation}
or in terms of spinors
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{4\pi K} \int dx d\tau \left( \psi^\dagger \partial_\tau \psi + \psi^\dagger \sigma^x \sigma^y \partial_x \psi \right)\\
		&=  \frac{1}{4\pi K} \int dxd\tau \left( \psi^\dagger \sigma^x\sigma^x\partial_\tau \psi + \psi^\dagger \sigma^x \sigma^y \partial_x \psi \right)\\
		&=  \frac{1}{4\pi K} \int dxd\tau \ \bar{\psi} \slashed{\partial} \psi
	\end{aligned}
\end{equation}
Note that the integral meausre also contriutes a $1/2$ factor. 

The two point correlation function reads 
\begin{equation}
	\begin{aligned}
		&\langle \psi_{+}(z) \, \psi_{+}(w) \rangle = K\frac{1}{z-w} \\
		&\langle \psi_{-}(z) \, \psi_{-}(w) \rangle = K\frac{1}{\bar{z}-\bar{w}} \\
		&\langle \psi_{+}(z) \, \psi_{-}(w) \rangle = 0\\
	\end{aligned}
\end{equation} 
...
\begin{equation}
	j^\mu \sim -i\bar{\psi} \gamma^\mu \left(i\psi\right) = \bar{\psi} \gamma^\mu \psi.
\end{equation}
The charge density is the total fermion charge density $j^0 \sim \psi^\dagger \psi$.
...
\begin{equation}
	j_a^\mu \sim \bar{\psi} \gamma^\mu\gamma^5 \psi
\end{equation}
whose charge density reads $j^0_a \sim \psi^\dagger \gamma^5 \psi = \psi_{+}^\dagger \psi_{+} - \psi_{-}^\dagger \psi_{-}$. It is the difference between the left and right movers.
...
\begin{equation}
	\begin{aligned}
		S &= \frac{1}{8\pi K}\int dz d\bar{z} \frac{1}{2} \left( 2 \partial \varphi \bar{\partial} \varphi + 2 \bar{\partial} \varphi \partial \varphi \right) \\
		&= \frac{1}{4\pi K}\int dz d\bar{z} \left( \partial \varphi \bar{\partial} \varphi \right)
	\end{aligned}
\end{equation}
...
\end{equation}
The vertex operators have conformal dimensions $h(\alpha)=\bar{h}(\alpha) = \frac{\alpha^2}{8\pi g} = \frac{\alpha^2 K}{4}$.
...
\begin{equation}
	\begin{split}
		\phi(z) &= \phi_0 -i\,j_0\,\ln \,z + i\,\sum_{n\neq0} \frac{1}{n}\,j_n\,z^{-n} \\
		\bar{\phi}(\bar{z}) &= \bar{\phi}_0 -i\,\bar{j}_0\,\ln \, \bar{z} + i\,\sum_{n\neq0} \frac{1}{n}\,\bar{j}_n\,{\bar{z}}^{-n}.
	\end{split}
\end{equation} 
Note here the complex coordinate denotes $z = e^\omega = e^{\tau + i\sigma}$. $\omega$ is the coordinate defined on the cylinder. Whenever we use a quantum theory, we should consult to the radial quantization. In terms of the chiral term, we can write down the free scalar $\varphi = \phi + \bar{\phi}$ and its dual field $\theta = \phi - \bar{\phi}$ as
\begin{equation}
	\begin{split}
		\varphi &= \varphi_0 -i \tau \left(j_0 + \bar{j_0}\right) + \sigma \left(j_0 - \bar{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}+ \bar{j}_n\,{\bar{z}}^{-n}\right)\\
		\theta &= \theta_0 -i \tau \left(j_0 - \bar{j_0}\right) + \sigma \left(j_0 + \bar{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}- \bar{j}_n\,{\bar{z}}^{-n}\right).
	\end{split}
\end{equation}
...
\begin{equation}
	H = \int dx \left( 2\pi \Pi^2 + \frac{1}{8\pi} {\left(\partial_x \phi\right)}^2 \right),
\end{equation}
where the canonical momentum $\Pi = \frac{1}{4\pi} \dot{\phi}$. In terms of current operators we can write down (note $x := \sigma$ in the Hamiltonian is defind on the cylinder)
\begin{equation}
	\begin{aligned}
		\partial_x \varphi &= \partial \varphi\ i z + \bar{\varphi} (-i) z = j_n/z^n -\bar{j}_n / \bar{z}^n \\
		4\pi \Pi &= \partial \varphi i z + \bar{\partial} \varphi i \bar{z} = j_n/z^n + \bar{j}_n / \bar{z}^n
	\end{aligned}
\end{equation}
Thus the Hamiltonian can be written down in terms of current operators
\begin{equation}
	\begin{aligned}
		H &= \frac{1}{4\pi} \int_0^{2\pi} d\sigma \sum_{n,m} \left( \frac{j_n j_m}{z^{n+m}} + \frac{\bar{j}_n \bar{j}_m}{{\bar{z}}^{n+m}} \right) \\
		&= \frac{1}{2} j_0 j_0 + \frac{1}{2} \bar{j}_{0} \bar{j}_0 + \sum_{n>0} j_{-n} j_n + \bar{j}_{-n} \bar{j}_n.
	\end{aligned}
\end{equation}
We thus identify the Virasoro generator
\begin{equation}
	\begin{aligned}
		L_0 &= \frac{1}{2} j_0 j_0 + \sum_{n>0} j_{-n} j_n \\
		\bar{L}_0 &= \frac{1}{2} \bar{j}_{0} \bar{j}_0 + \sum_{n>0} \bar{j}_{-n} \bar{j}_n.
	\end{aligned}
\end{equation}

It is clear that $j_0$ and $\bar{j}_0$ are converved quantuties. In the following discussions, we will use the notation $j_0$ interchangably to represent both the operator and its charge. Now let's derive their charges in the quanttum theory. This relies on the modular invariance of the torus partition function. Thanks to the current operator algebra, we can construct the Hilbert space as
\begin{equation}
...
\begin{equation}
	Z_{cir}(q,\bar{q}) = q^{j_0^2/2} \, \bar{q}^{\bar{j}_0^2/2} \, Z^{\prime}_{boson} (q,\bar{q}),
\end{equation}
where $Z^\prime_{boson}$ denotes the part without zero modes. A modular $T$ transformation $\tau / \bar{\tau} \rightarrow \tau / \bar{\tau} + 1$ induces $q \rightarrow e^{2\pi i}q$ and $\bar{q} \rightarrow e^{-2\pi i}\bar{q}$. The partition function becomes
\begin{equation}
	Z_{cir}(e^{2\pi i}q,e^{-2\pi i}\bar{q}) = e^{2\pi i \left(j_0^2 - \bar{j}_0^2\right)/2} \, q^{j_0^2/2} \, \bar{q}^{\bar{j}_0^2/2} \, Z^{\prime}_{boson} (q,\bar{q}).
\end{equation}
A modular invariant partition function thus requires 
\begin{equation}
	j_0^2 - \bar{j}_0^2 = 2k,
\end{equation}
where $k$ is an integer. On the hand, since the theory is defined on a circle, the scalar $\varphi(z,\bar{z})$ is single valued when changing $z$ to $ze^{i2\pi}$. This resultes in another the relation
\begin{equation}
	j_0 - \bar{j}_0 = m\,R.
\end{equation}
where $m$ is an integer denoting the {\it magnetic} charge. Note that the second constraint is a factor of the first one, 
\begin{equation}
	j_0^2 - \bar{j}_0^2 = \frac{2k}{m\,R} \ m R = \frac{2k}{m} m
\end{equation}
...
\begin{equation}
	\begin{aligned}
		j_0 &= e \frac{1}{R} + m \frac{R}{2} \\
		\bar{j}_0 &= e \frac{1}{R} - m \frac{R}{2}.
	\end{aligned}
\end{equation}

From the quantization of zero modes $j_0$, it is also clear that the dual scalar $\theta(x)$ is compactified as $\theta \sim \theta + 4\pi/R$.

{\bf Symmetries in free compact boson--.}
...
\begin{equation}
	\begin{aligned}
		&j^0_\varphi \sim \partial^t\varphi = i \left(\partial \varphi + \bar{\partial} {\varphi}\right) \\
		&j^1_\varphi \sim \partial^x\varphi = -i\left( \partial \varphi - \bar{\partial}\varphi \right)
	\end{aligned}
\end{equation}
The charge $Q_\varphi$ (electric) is 
\begin{equation}
	Q_\varphi \sim j_0 + \bar{j}_0 = 2e / R.
\end{equation}
...
\begin{equation}
	S = \frac{1}{8\pi K}\int \left(\partial \theta\right)^2
\end{equation}
...
\begin{equation}
	\begin{aligned}
		&j^0_\theta \sim \partial^t\theta = i \left(\partial \theta + \bar{\partial} {\theta}\right) = -\partial^x \varphi = -j^1_\varphi\\
		&j^1_\theta \sim \partial^x\theta = -i\left( \partial \theta - \bar{\partial}\theta \right) = -\partial^t \varphi = -j^0_\varphi
	\end{aligned}
\end{equation}
The charge for the $\theta$ scalar, the magnetic charge, reads
\begin{equation}
	Q_\theta \sim j_0 - \bar{j}_0 = m \, R.
\end{equation}


{\bf Bosonization dictonary for free fermions---}.
The simplest way for bosonization of fermions is through the vertext operator $V_\alpha = :e^{i\alpha \phi(z)}:$ with a dimension $(\alpha^2/2,0)$. Note that due to the compact nature of the free scalar, physical operators are expressed in terms of $e^{i\varphi}$ or $e^{i\theta}$. The vertext operator thus can be written as
\begin{equation}
	V_\alpha \sim e^{i\alpha\phi} \sim e^{i\alpha\varphi/2} e^{i\alpha\theta/2} 
\end{equation}
Now we can bosonize the fermion fields
\begin{equation}
	\begin{split}
		&\psi^{\dagger}_{+} \sim e^{i \phi} \\
		&{\psi}^{\dagger}_{-} \sim e^{i\bar{\phi}}.
	\end{split}
\end{equation}
...
\begin{equation}
	\rho(x) = \psi^\dagger \psi \sim i \left(\partial \varphi + \bar{\partial} \varphi\right) \sim j^0_\varphi.
\end{equation}
...
\begin{equation}
	\rho_a = \psi^\dagger \gamma^5 \psi \sim \left( \psi^{\dagger}_{+} \psi_{+} - \psi^{\dagger}_{-}\psi_{-} \right) \sim i \left(\partial \theta - \bar{\partial} \theta\right) \sim j^0_\theta
\end{equation}
which is the magnetic charge. 

A mass term is mapped to be 
\begin{equation}
	\bar{\psi} \psi \sim \psi_{+}^\dagger \psi_{-} + \psi_{-}^\dagger \psi_{+} \sim e^{i\theta} + h.c. \sim \cos\left(\theta\right)
\end{equation}
whose dimensional is $\left(1/2,1/2\right)$.

The density-density interaction $\rho_{+} \rho_{-}$ is mapped to 
\begin{equation}
	\rho_{+} \rho_{-} \sim -\partial \varphi \bar{\partial} \varphi,
\end{equation}
...
And similarly 
\begin{equation}
	\rho_{-} \rho_{-} \sim -\left(\bar{\partial} \varphi \right)^2.
\end{equation}
