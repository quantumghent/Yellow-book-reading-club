...
{\bf Wick rotation.---}
It is usually more convenient to work in Euclidean space rather than Minkowski space time. We first assume the Wick rotation acts as $t \rightarrow i\tau$. The left mover $\psi_{+} \sim f(\tau-ix) \sim f(\overline{z})$. The action follows
\begin{equation}
...
\begin{equation}
	\begin{aligned}
		...
		&= \frac{1}{4\pi K}\int d^2x \,2\left(\psi_{+} {\partial} \psi_{+} + \psi_{-} \overline{\partial} \psi_{-} \right) \\
		&= \frac{1}{4\pi K}\int d^2z\, \left(\psi_{+} {\partial} \psi_{+} + \psi_{-} \overline{\partial} \psi_{-} \right) \\
	\end{aligned}
\end{equation}
The equation of motion tells us that, $\psi_{+}$ is antiholomorphic $\psi_{+} \sim f(\overline{z})$, consistent with the Wick rotation assumption. Respectively $\psi_{-}$ is a holomorphic field. One should be carful that we abuse the ustage of complex and real coordinates in the following. Namely we are going to use use $\psi_{+}(t+x)$ and $\psi_{+}(\overline{z})$.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Note that the integral meausre also contriutes a $1/2$ factor. This fix our convention of $\gamma$ matrices in Euclidean space. 

The two point correlation function reads 
\begin{equation}
	\begin{aligned}
		&\langle \psi_{-}(z) \, \psi_{-}(w) \rangle = K\frac{1}{z-w} \\
		&\langle \psi_{+}(\overline{z}) \, \psi_{+}(\overline{\omega}) \rangle = K\frac{1}{\overline{z}-\overline{w}} \\
...
\begin{equation}
	j^\mu \sim -i\overline{\psi} \gamma^\mu \left(i\psi\right) = \overline{\psi} \gamma^\mu \psi.
\end{equation}
...
\begin{equation}
S = \int d^d x \ \left\{\frac{1}{2}{\left(\Delta\phi\right)}^2 + \sum_{n=1,2,4}\left(\frac{\lambda_n}{n!}\phi^n\right) \right\}
\end{equation}
...
\noindent {\bf Example: perturbative RG analysis of BKT transition}
...
\begin{equation}
	\begin{split}
		...
		\overline{\phi}(\overline{z}) &= \overline{\phi}_0 -i\,\overline{j}_0\,\ln \, \overline{z} + i\,\sum_{n\neq0} \frac{1}{n}\,\overline{j}_n\,{\overline{z}}^{-n}.
	\end{split}
\end{equation} 
...
\begin{equation}
	\begin{split}
		\varphi &= \varphi_0 -i \tau \left(j_0 + \overline{j_0}\right) + \sigma \left(j_0 - \overline{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}+ \overline{j}_n\,{\overline{z}}^{-n}\right)\\
		\theta &= \theta_0 -i \tau \left(j_0 - \overline{j_0}\right) + \sigma \left(j_0 + \overline{j_0}\right) + i\,\sum_{n\neq0} \frac{1}{n}\,\left( j_n\,z^{-n}- \overline{j}_n\,{\overline{z}}^{-n}\right).
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\partial_x \varphi &= \partial \varphi\ i z + \overline{\varphi} (-i) z = j_n/z^n -\overline{j}_n / \overline{z}^n \\
		4\pi \Pi &= \partial \varphi i z + \overline{\partial} \varphi i \overline{z} = j_n/z^n + \overline{j}_n / \overline{z}^n
	\end{aligned}
\end{equation}
Thus the Hamiltonian can be written down in terms of current operators
\begin{equation}
	\begin{aligned}
		H &= \frac{1}{4\pi} \int_0^{2\pi} d\sigma \sum_{n,m} \left( \frac{j_n j_m}{z^{n+m}} + \frac{\overline{j}_n \overline{j}_m}{{\overline{z}}^{n+m}} \right) \\
...
\begin{equation}
	\begin{aligned}
		...
		\overline{L}_0 &= \frac{1}{2} \overline{j}_{0} \overline{j}_0 + \sum_{n>0} \overline{j}_{-n} \overline{j}_n.
	\end{aligned}
\end{equation}
...
\begin{equation}
	Z_{cir}(q,\overline{q}) = q^{j_0^2/2} \, \overline{q}^{\overline{j}_0^2/2} \, Z^{\prime}_{boson} (q,\overline{q}),
\end{equation}
where $Z^\prime_{boson}$ denotes the part without zero modes. A modular $T$ transformation $\tau / \overline{\tau} \rightarrow \tau / \overline{\tau} + 1$ induces $q \rightarrow e^{2\pi i}q$ and $\overline{q} \rightarrow e^{-2\pi i}\overline{q}$. The partition function becomes
\begin{equation}
...
\end{equation}
where $k$ is an integer. On the hand, since the theory is defined on a circle, the scalar $\varphi(z,\overline{z})$ is single valued when changing $z$ to $ze^{i2\pi}$. This resultes in another the relation
\begin{equation}
	j_0 - \overline{j}_0 = m\,R.
\end{equation}
...
\begin{equation}
	j_0^2 - \overline{j}_0^2 = \frac{2k}{m\,R} \ m R = \frac{2k}{m} m
\end{equation}
...
\begin{equation}
	\begin{aligned}
		...
		\overline{j}_0 &= e \frac{1}{R} - m \frac{R}{2}.
	\end{aligned}
\end{equation}
...
\begin{equation}
	Q_\varphi \sim j_0 + \overline{j}_0 = 2e / R.
\end{equation}
...
\begin{equation}
	Q_\theta \sim j_0 - \overline{j}_0 = m \, R.
\end{equation}
...
\begin{equation}
	\begin{split}
		...
		&{\psi}^{\dagger}_{+} \sim e^{i\overline{\phi}}.
	\end{split}
\end{equation}
...
\begin{equation}
	E_k \sim -\cos (k),
\end{equation}
one can identify fermions around $\mp k_f$ as the left/right mover $\psi_{+}$/$\psi_{-}$.
The slow modes are close to $\pm k_f$ ($k_f > 0$),
\begin{equation}
	\begin{aligned}
		c_{t,x} \sim e^{-ik_f \left( t+x \right)} \psi_{+} + e^{ik_f\left(t-x\right)} \psi_{-}
	\end{aligned}
\end{equation} 
