...
{\bf Complex Coordinate.--}  In $1+1d$ Minkoswki space, fields are functions of $x-t$ or $x+t$. In CFT, there are two independent Virasoro algebras, in which the holomorphic (resp. antiholomorphic) part has fields $\phi(x-t)$ ($\phi(x+t)$). It is more convenient to use imaginary time $\tau = it$ and complex coordinates. It is defined as 
\begin{equation}
\begin{aligned}
z &= z^0 + i z^1 \\
\bar{z} &= z^0 - i z^1.
\end{aligned}
\end{equation}  
Note that here $z$ and $\bar{z}$ denote coordinates with upper index. One can simply treat $z$ and $\bar{z}$ as ordinary upper coordinates and write down the fields with the new coordinate. 
...
\begin{equation}
	G(r) = \langle \phi(x) \phi(y) \rangle = 2\pi K \int \frac{d^2k}{(2\pi)^2}\, \frac{e^{ikr\,\mathrm{cos}(\theta)}}{k^2}
\end{equation}
