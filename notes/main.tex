...
Besides the closed manifolds, CFTs can also be defined on manifols with boundaries, or defects in more general setting. A boundary can be understood a special defect between a CFT and a trivial CFT with a central charge $c=0$. On the other hand a defect between CFT1 and CFT2 can be regarded as a boundary of the stacking the two CFTs using the folding trick. It is natural to require the conformal symmetry still holds at boundaries and defects. For a boundary state $\vert a \rangle$ this means 
\begin{equation}
	L_n - \bar{L}_{-n} \vert a \rangle = 0.
\end{equation}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
which are called topological defects. The topological defects correspond to the topological aspects of CFT. A full understanding of CFT must also include the knowledge of boundaries and defects.
