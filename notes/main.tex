...
Note the replacement of $\lambda$ with $g$ generates terms with four loops, which means we can safely express bare quantities in terms of $g$ and $m^2$. The vertex functions are finite at the two-loop level when $d \leq 4$ with the chosen renormalization conditions
\begin{equation}
	\begin{aligned}
		&\Gamma_R^{(E)}(k_i,m^2,g) = Z_\phi^{E/2}(k_i,m_0^2,\lambda,\Lambda) \\
