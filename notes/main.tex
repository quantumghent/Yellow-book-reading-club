...
In Minkowski space time,  for the free fermion reads
\begin{equation}
    \mathcal{L} = \frac{g}{2}\left(\psi^1\ i(\partial t - \partial x)\ \psi^1 + \psi^2\ i(\partial t + \partial x)\ \psi^2 \right).
\end{equation}
In terms of $\psi = (\psi^1,\psi^2)^t$, one can write the theory as
\begin{equation}
	\begin{aligned}
...
\begin{equation}
    S = \frac{1}{8\pi\,K}\int \ dxdt \ \partial_\mu \phi \partial^\mu \phi,
\end{equation}
where $\phi$ is a real scalar field. After Wick rotation $\tau = it$, it becomes
\begin{eqnarray}
    i\ S &=& \frac{i}{8\pi\,K}\int \ dxdt \ \partial_t \phi \partial_t \phi - \frac{i}{2}g\int \ dxdt \ \partial_x \phi \partial_x \phi \nonumber \\
    &=& -\frac{1}{8\pi\,K}\int \ dxd\tau \ \partial_\tau \phi \partial_\tau \phi - \frac{1}{2}g\int \ dxdit \ \partial_x \phi \partial_x \phi \nonumber \\
    &=& -\frac{1}{8\pi\,K}\int \ dxd\tau \ \partial_\mu \phi \partial^\mu \phi
\end{eqnarray}
The Euclidean action reads
\begin{equation}
    S_E = \frac{1}{8\pi\,K}\int \ d^2x \ \partial_\mu \phi \partial^\mu \phi.
\end{equation}
...
\begin{equation}
    \langle \phi(x) \phi(y) \rangle = -2 K\,\mathrm{\log} (r),
\end{equation}
...
\begin{equation}
	S_E = \frac{1}{4\pi\,K} \int\frac{d^2k}{(2\pi)^2} \, \phi_k \left(\frac{k^2}{2}\right)\phi_{-k}.
\end{equation} 
One finds the real-space two-point correlation function
\begin{equation}
	G(r) = \langle \phi(x) \phi(y) \rangle = 4\pi K \int \frac{d^2k}{(2\pi)^2}\, \frac{e^{ikr\,\cos(\theta)}}{k^2}
\end{equation}
...
\begin{equation}
	\langle\, V_\alpha(x)\, V_{-\beta}(y)\, \rangle = e^ {\alpha^2 \langle \phi(x) \phi(y) \rangle} \sim \frac{\delta_{\alpha,\beta}}{r^{2\alpha^2 K}}.
\end{equation}
...
One can also write down the Hamiltonian for the free boson. The canonical momentum corresponding to $\phi$ is $\pi = \frac{\partial L}  {\partial \dot\phi} = \frac{\dot{\phi}}{4\pi K}$. One finds the Hamiltonian is 
\begin{equation}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
The chiral two-point correlation can be deduced from the physical one to be $\langle \phi(z) \phi(0) \rangle \sim = -K \log(z) $.
...
With the chiral mass center introduced above the chiral part of the compact boson field $\varphi(z,\bar{z}) = \phi(z) + \bar{\phi}(\bar{z})$ can be expanded in terms of current operators. Recall that the two-point correlation of free boson $\langle \varphi (x) \varphi(y) \rangle = -2 K \log (r)$. The current operator can be defind as 
\begin{equation}
...
\begin{equation}
	\begin{split}
		\phi(z) &= \sqrt{K}\left(\phi_0 -i\,j_0\,\mathrm{ln}\,z + i\,\sum_{n\neq0} \frac{1}{n}\,j_n\,z^{-n}\right) \\
